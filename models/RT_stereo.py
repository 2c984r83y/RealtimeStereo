...
def convbn(in_planes, out_planes, kernel_size, stride, pad, dilation, groups=1):
    return nn.Sequential(nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
                                   padding=dilation if dilation > 1 else pad, dilation=dilation, groups=groups,
                                   bias=False),
    ...
    def _build_volume_2d(self, feat_l, feat_r, maxdisp, stride=1):
        ...
        for i in range(0, maxdisp, stride):
            if i > 0:
                ...
                # 左图的x坐标更大, 右图的x坐标更小
                cost[:, :, i // stride, :, i:] = torch.norm(feat_l[:, :, :, i:] - feat_r[:, :, :, :-i], p=1, dim=1,
                                                            keepdim=True)
            else:
                cost[:, :, i // stride, :, i:] = torch.norm(feat_l[:, :, :, :] - feat_r[:, :, :, :], p=1, dim=1,
                                                            keepdim=True)  # L1 Norm
...
class disparityregression2(nn.Module):
    def __init__(self, start, end, stride=1):
        ...
        self.disp = torch.arange(start * stride, end * stride, stride).view(1, -1, 1, 1).type(
            torch.FloatTensor).cuda().requires_grad_(False)
